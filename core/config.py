...
class Config:
    ...
    def _validate_required(self):
        ...
        if empty:
            if self.logger:
                self.logger.error(CONFIG, f"Required config fields cannot be empty: {', '.join(empty)}")
    ...
    def _validate_optional_booleans(self):
        for key, default in self.OPTIONAL_FIELDS_WITH_DEFAULTS.items():
            ...
            if value == "":
                if self.logger:
                    self.logger.warn(CONFIG, f"Optional field '{key}' is empty. Using default: {default}")
                self._data[key] = default
                continue

            if not isinstance(value, bool):
                if self.logger:
                    self.logger.warn(CONFIG, f"Optional field '{key}' must be a boolean. Got {value!r}. Using default: {default}")
                self._data[key] = default
                continue
    ...
    def _validate_devices(self):
        ...
        if not isinstance(devices, list):
            if self.logger:
                self.logger.error(CONFIG, f"'devices' field must be a list. Got {type(devices).__name__}.")
            raise ConfigError(f"'devices' field must be a list. Got {type(devices).__name__}.")

        for idx, device in enumerate(devices):
            if not isinstance(device, dict):
                if self.logger:
                    self.logger.error(CONFIG, f"Device entry at index {idx} must be an object. Got {type(device).__name__}.")
            ...
            for field in required_device_fields:
                if field not in device or not device[field]:
                    if self.logger:
                        self.logger.error(CONFIG, f"Device entry at index {idx} is missing required field '{field}' or it is empty.")
                    raise ConfigError(f"Device entry at index {idx} is missing required field '{field}' or it is empty.")
